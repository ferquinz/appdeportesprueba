...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		...
		$query=$db->query("select * from ".$tabla." where fecha='".$_GET["fecha"]."' AND id_customer=".$_SESSION['id_customer']."
		AND id_team=".$_SESSION['id_team']." order by id asc");
	...
	case "guardar_evento":
	{
		...
		$query=$db->query("insert into ".$tabla." (fecha,evento, id_customer, id_team) values ('".$_GET["fecha"]."','".strip_tags($_GET["evento"])."',".$_SESSION['id_customer']."
		,".$_SESSION['id_team'].")");
	...
	case "generar_calendario":
	{
		...
		$query=$db->query("select fecha,count(id) as total from ".$tabla." where month(fecha)='".$fecha_calendario[1]."' and year(fecha)='".$fecha_calendario[0]."'
			AND id_customer=".$_SESSION['id_customer']."
			AND id_team=".$_SESSION['id_team']." group by fecha");
		if ($fila=$query->fetch_array())
		{
			do
			{
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				if ($tr<$totalfilas)
				{
					if ($i>=$primeromes && $i<=$tope) 
					{
						...
						if (date("Y-m-d")<=$fecha_completa) echo "<a href='#' data-evento='#nuevo_evento' title='Agregar un Evento el ".fecha($fecha_completa)."' class='add agregar_evento' rel='".$fecha_completa."'>&nbsp;</a>";
